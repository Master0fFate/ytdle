...
## ⚠ Due to YT constantly blocking downloads and yt-dlp updating almost weekly, the .exe's are most likely outdated(for YT downloads at least), so i suggest installing python on your machine and running the script itself.
...
> ffmpeg is required so if you want to run it as a python script, additional to other requirements are PySide6 and yt_dlp, you have to have ffmpeg either as a system PATH or an .exe in the same directory.
